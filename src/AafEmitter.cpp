...
void AafEmitter::emit(const CompositionData &comp) {
...
void AafEmitter::emitMarkers(const std::vector<MarkerData> &markers) {
    for ( const auto &m : markers ) {
        m_writer.writeMarker(m.id, m.t, m.name.c_str(), m.isRegion, m.color);
        if ( m.isRegion )
            m_writer.writeMarker(m.id, m.endT, "", true, m.color);
    }
}

void AafEmitter::emitAudioTrack(const AudioTrackData &track) {
...
void AafEmitter::emitVideoTrack(const VideoTrackData &track) {
    auto w_trk = m_writer.track("VIDEO", 1.0, 0.0, 0, 0, 1);
    for ( const auto &clip : track.clips )
        emitVideoClip(clip);
}

void AafEmitter::emitClip(const ClipData &clip) {
    auto w_itm =
...
void AafEmitter::emitEnvelope(const EnvelopeData &env) {
    auto w_env = m_writer.envelope(env.tag.c_str(), env.arm);
    for ( const auto &pt : env.points )
        m_writer.writeEnvPoint(pt.t, pt.value);
}

void AafEmitter::emitSource(const SourceData &source) {
...
void AafEmitter::emitVideoClip(const VideoClipData &clip) {
